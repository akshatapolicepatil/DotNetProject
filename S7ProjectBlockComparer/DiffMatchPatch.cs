    ...
    public class diff_match_patch
    {
                ...
                switch (diffs[pointer].operation)
                {
                    ...
                    case Operation.EQUAL:
                        ...
                        if (count_delete >= 1 && count_insert >= 1)
                        {
                            ...
                            List<Diff> a = this.diff_main(text_delete, text_insert, false, deadline);
        ...
        public void diff_cleanupEfficiency(List<Diff> diffs)
        {
            ...
            while (pointer < diffs.Count)
            {
                ...
                else
                {
                     ...
                     */
                    if ((lastequality.Length != 0) && ((pre_ins && pre_del && post_ins && post_del) || ((lastequality.Length < this.Diff_EditCost / 2) && ((pre_ins ? 1 : 0) + (pre_del ? 1 : 0) + (post_ins ? 1 : 0) + (post_del ? 1 : 0)) == 3)))
                    {
                        // Duplicate record.
                        diffs.Insert(equalities.Peek(), new Diff(Operation.DELETE, lastequality));
                        // Change second copy to insert.
                ...
                switch (diffs[pointer].operation)
                {
                    ...
                    case Operation.EQUAL:
                        ...
                        if (count_delete + count_insert > 1)
                        {
                            ...
                            if (count_delete == 0)
                            {
                                diffs.Splice(pointer - count_insert, count_delete + count_insert, new Diff(Operation.INSERT, text_insert));
                            }
                            else if (count_insert == 0)
                            {
                                diffs.Splice(pointer - count_delete, count_delete + count_insert, new Diff(Operation.DELETE, text_delete));
                            }
                            else
                            {
                                diffs.Splice(pointer - count_delete - count_insert, count_delete + count_insert, new Diff(Operation.DELETE, text_delete), new Diff(Operation.INSERT, text_insert));
                            }
        ...
        protected int match_bitap(string text, string pattern, int loc)
        {
            ...
            if (best_loc != -1)
            {
                ...
                best_loc = text.LastIndexOf(pattern, Math.Min(loc + pattern.Length, text.Length), StringComparison.Ordinal);
                if (best_loc != -1)
                {
                    score_threshold = Math.Min(match_bitapScore(0, best_loc, loc, pattern), score_threshold);
                }
        ...
        protected void patch_addContext(Patch patch, string text)
        {
            ...
            while (text.IndexOf(pattern, StringComparison.Ordinal) != text.LastIndexOf(pattern, StringComparison.Ordinal) && pattern.Length < Match_MaxBits - Patch_Margin - Patch_Margin)
            {
            ...
            foreach (Diff aDiff in diffs)
            {
                ...
                switch (aDiff.operation)
                {
                    ...
                    case Operation.DELETE:
                        ...
                        patch.diffs.Add(aDiff);
                        postpatch_text = postpatch_text.Remove(char_count2, aDiff.text.Length);
                        break;
                    case Operation.EQUAL:
                        ...
                        else
                        {
                            ...
                            foreach (Diff aDiff in aPatch.diffs)
                            {
                                if (aDiff.operation != Operation.EQUAL)
                                {
                                    ...
                                    {
                                        // Deletion
                                        text = text.Remove(start_loc + index2, diff_xIndex(diffs, index1 + aDiff.text.Length) - index2);
                                    }
                                }
                                if (aDiff.operation != Operation.DELETE)
                                {
            ...
            for (int x = 0; x < patches.Count; x++)
            {
                ...
                while (bigpatch.diffs.Count != 0)
                {
                    ...
                    while (bigpatch.diffs.Count != 0 && patch.length1 < patch_size - this.Patch_Margin)
                    {
                        ...
                        else if (diff_type == Operation.DELETE && patch.diffs.Count == 1 && patch.diffs.First().operation == Operation.EQUAL && diff_text.Length > 2 * patch_size)
                        {
